...
### 2.9 按比例拼柜 Set Load（`RATIO_MODE`）

**需求**：多款外箱锁定配比（如 外箱1 : 外箱2 = 1 : 2），测算**单柜最多能装多少完整套**。

**核心原则：零侵入。** 不改动 `packContainer` 的任何摆放逻辑，只在其外层套一层「套数搜索」，
因此摆放位置、朝向枚举、重心校核与原规则**完全一致**。

```
① expandUnits() 给每个 unit 附 ratio（正整数，默认 1）
② 套数上界 kUp：min( ⌊柜容×0.95 / 每套体积⌋, ⌊柜载重 / 每套重量⌋ )
   containerType='auto' 时对 4 种柜型各算 kUp，取最大者作为目标柜
③ 在 [1, kUp] 上二分 k：
     tryRatioFit(container, units, ratios, k) → 各 unit.qty = ratios[i] × k
     → packContainer() → 统计 counts，全部达标则 k 可行
④ 二分收敛后再向上线性补探 +1..+3，补偿启发式算法可能出现的非单调
⑤ RATIO_FILL（默认关）→ fillRatioFit() 补装
```

**补装策略（`fillRatioFit`）—— 关键坑位**：
不能「先不限量装一柜、再反推套数」，那样算法会优先抢装好装的款，导致配比失衡、
**可配套数反而暴跌**（实测 1:1 用例从 224 套掉到 128 套）。正确做法是：
把其他款的配额锁定为 `ratio × k`，**只放开其中一款**不限量去填剩余空间，逐款试一遍，
仅接受「套数不下降 **且** 箱数增加」的方案，取箱数最多者。若没有一款能多塞（柜子确实满了），
`fillUnit` 保持 `null`，结果等同严格模式。

**性能**：约 10~16 次 `packContainer` 调用，实测 40HQ / 1:2 用到 10 次、耗时 **296ms**。
`runRatioOptimize()` 是 `async`，每次试装后 `await ratioUIYield()`（`requestAnimationFrame + setTimeout`）
让出主线程刷新 `#ratioBusy` 提示，界面不假死。

**实测基准**（40HQ，`PACK_CFG.marginPct=1.5`）：

| 用例 | 比例 | 严格套数 | 箱数 | 装载率 | 补装后 |
|---|---|---|---|---|---|
| 60×40×30(8kg) : 50×35×25(5kg) | 1 : 2 | **402** | 1206 | 84.0% | 402（柜已满，无余位） |
| 80×60×50(18kg) : 40×30×25(4kg) | 1 : 1 | **224** | 448 | 79.2% | 224 套 / **592** 箱（+144）84.9% |
| 40×40×30 : 48×48×34 : 56×56×38 | 4 : 3 : 2 | 90 | 810 | — | — |

**已知约束**：
- 只支持单柜测算（不做多柜拆分）；套数上限硬顶 20000，避免极端输入死循环。
- `type='set'`（套装 Set）产品不参与按比例拼柜，会被忽略并在结果面板给出提示
  （套装本身就是「一套」，再叠加比例会语义冲突）。
- 二分依赖「套数可行性单调」假设；heuristic 装箱存在少量非单调，故用第 ④ 步的线性补探兜底（最多 +3）。
...
7. **按比例拼柜的补装反噬**：绝不能用「先不限量装、再反推套数」实现补装 —— 实测会让可配套数暴跌（224→128）。必须锁死其他款、只放开一款填缝，见 §2.9。
8. **比例输入框不能触发 `refreshProducts()`**：比例框的 `oninput` 只更新内存并刷新顶部比例串（`updateRatioSummary`），一旦重渲染产品卡输入框就会失焦，用户无法连续输入。
...
- Mode A 按比例拼柜：勾选「按比例拼柜」→ 各产品卡「数量」变「比例」→ 填 1 : 2 →
  点「计算最优方案」→ 应显示最大套数（40HQ + 60×40×30 : 50×35×25 @1:2 = **402 套**），
  各款箱数 = 比例 × 套数（严格模式无超装）。
